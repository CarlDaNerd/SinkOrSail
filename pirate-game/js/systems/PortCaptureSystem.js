// ── systems/PortCaptureSystem.js ── (replaces Home Base)
// You don't BUILD a home base — you TAKE one. Shell a port (player cannonballs
// only) until its hull drops below PORT_CAPTURE_THRESHOLD_PCT, then press B
// while close to capture it the way you board a ship. A captured port flips to
// owner:'player': it heals back up, its towers stop firing on you, and it
// becomes a home for runners (M5) plus the usual dock / bank / repair.
//
...
//
// Shares the B key with ship capture; PortCaptureSystem.tryCapture is checked
// first (you're either next to a port or next to a ship, not both).
const PortCaptureSystem = {
  init(scene){
    scene.ownedPorts = scene.ownedPorts || [];
    if (!scene._captureKey) scene._captureKey = scene.input.keyboard.addKey('B');
  },

  // a player shot landed on this port
  ...
  // hull bar over any damaged or owned port
  draw(scene, g){
    for (const port of scene.navyPorts){
