...
class HUD {
  constructor(scene, gs){
    ...
    // chaser indicators (horizontal bars, distinct from the vertical broadside bars)
    this.tChaseBow   = mk(252, 104, 8, '#C8A86a');
    this.tChaseStern = mk(252, 121, 8, '#6aA8C8');
  ...
  update(pl, wa){
    ...
    // ── chaser indicators: horizontal bars, each against its OWN cooldown so the
    // bow (faster) and stern (slower) read differently from each other and from
    // the vertical broadside bars. Only shown when the tier mounts that chaser. ──
    const hasBow   = (typeof ShipTiers !== 'undefined') && ShipTiers.has(pl, 'bow');
    const hasStern = (typeof ShipTiers !== 'undefined') && ShipTiers.has(pl, 'stern');
    const cbX = 200, cbW = 48, cbH = 7;
    const chaseBar = (y, val, cdRef, lit) => {
      const fill = 1 - Math.min(1, Math.max(0, val)/cdRef);
    ...
    if (hasBow){
      const cd = (typeof ShipTiers !== 'undefined') ? ShipTiers.cooldown('bow') : 1;
      chaseBar(104, pl.fire.bow, cd, 0xC8A86a); this.tChaseBow.setText('BOW').setVisible(true);
    } else this.tChaseBow.setVisible(false);
    if (hasStern){
      const cd = (typeof ShipTiers !== 'undefined') ? ShipTiers.cooldown('stern') : 1;
      chaseBar(122, pl.fire.stern, cd, 0x6aA8C8); this.tChaseStern.setText('STERN').setVisible(true);
    } else this.tChaseStern.setVisible(false);
