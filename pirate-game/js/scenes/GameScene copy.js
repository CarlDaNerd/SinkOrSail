...
class GameScene extends Phaser.Scene {
  ...
  update(time, delta){
    ...
    if (pl.hull > 0){
      ...
      // ── firing: arrows are the primary scheme (4-way); Q/E kept as broadside aliases ──
      if (Phaser.Input.Keyboard.JustDown(this.cursors.left)  || Phaser.Input.Keyboard.JustDown(this.keys.Q)) Combat.fire(this, pl, 'port');
      if (Phaser.Input.Keyboard.JustDown(this.cursors.right) || Phaser.Input.Keyboard.JustDown(this.keys.E)) Combat.fire(this, pl, 'star');
      ...
      if (this.keys.A.isDown) pl.heading = (pl.heading - td + 360)%360;     // steering: A/D only now
      if (this.keys.D.isDown) pl.heading = (pl.heading + td)%360;
      ...
      if (pl.fire.bow > 0) pl.fire.bow -= dts; if (pl.fire.stern > 0) pl.fire.stern -= dts;
  ...
  drawShip(g, s){
    ...
    const sc = s.scale || 1;                              // tier visual scale
    g.scaleCanvas(sc, sc);
    ...
    // sails — one per mast, spaced along the keel (count from tier; default 1)
    if (s.sailState > 0){
      const h = s.sailState === 2 ? 18 : 10, nS = Math.max(1, s.sails || 1);
      g.fillStyle(0xD4C48C, 0.9);
      for (let m = 0; m < nS; m++){
        const my = -3 - m*((h + 6));                     // stack masts toward the bow
        g.fillRect(-9, my, 18, h);
      }
    }
    g.fillStyle(0x2A1404, 1); g.fillCircle(0, -3, 3.5);
    // bow / stern chaser muzzles (only drawn when the tier mounts them)
    if (typeof ShipTiers !== 'undefined'){
      g.fillStyle(0x15100A, 1);
      if (ShipTiers.has(s, 'bow'))   g.fillRect(-2, -22, 4, 6);   // forward muzzle at the bow
      if (ShipTiers.has(s, 'stern')) g.fillRect(-2, 16, 4, 6);    // aft muzzle at the stern
    }
